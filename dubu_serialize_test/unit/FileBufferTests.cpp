...
TEST(file_buffer, single_int) {
    ...
    {
        dubu::serialize::FileBuffer fileBuffer(
            fileName, dubu::serialize::FileBuffer::Mode::Write);
        WriteTest(fileBuffer, expectedValue);
        EXPECT_EQ(fileBuffer.Tell<int>(), 4);
    }
    {
        dubu::serialize::FileBuffer fileBuffer(
            fileName, dubu::serialize::FileBuffer::Mode::Read);
...
TEST(file_buffer, single_string) {
    ...
    {
        auto fileBuffer = dubu::serialize::FileBuffer(
            fileName, dubu::serialize::FileBuffer::Mode::Write);
        WriteTest(fileBuffer, expectedValue);
...
TEST(file_buffer, nested_vector_string) {
    ...
    std::vector<std::vector<std::string>> expectedValue{{"hello", "world"},
                                                        {"bye", "world"}};
    {
        dubu::serialize::FileBuffer fileBuffer(
            fileName, dubu::serialize::FileBuffer::Mode::Write);
...
TEST(file_buffer, custom_mesh_struct) {
    ...
    Mesh        expectedValue{.indices = {1, 2, 3},
                       .vertices{{3}, {2}, {1}},
                       .normals = {{2}, {1}, {3}}};
