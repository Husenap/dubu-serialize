...
ReadBuffer& operator>>(ReadBuffer& buffer, T& object) {
    internal::Serializer<T, std::is_arithmetic_v<std::decay_t<T>>>().Read(
        buffer, object);
    return buffer;
}
...
WriteBuffer& operator<<(WriteBuffer& buffer, const T& object) {
    internal::Serializer<T, std::is_arithmetic_v<std::decay_t<T>>>().Write(
        buffer, object);
    return buffer;
}
...
struct Serializer<std::basic_string<T>, false> {
    ...
    void Write(WriteBuffer& buffer, const std::basic_string<T>& object) {
        ...
        buffer << size;
        buffer.Write(reinterpret_cast<const char*>(&object[0]),
                     size * sizeof(T));
    }
};
